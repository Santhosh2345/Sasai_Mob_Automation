...
public class PHPMyAdminPage extends BasePage{
	...
	public PHPMyAdminPage(AndroidDriver driver) {
		super(driver);
	...
	public void clickGoQuery() throws Exception {
		UtilityCustomFunctions.doClick(Adriver, btnSubmitQuery);
	}
	public void setQuery(String sQuery) {
//		eleSQLTextWindow.sendKeys("test");
		
		UtilityCustomFunctions.sendKeys(Adriver, txtSQL, sQuery);
	}
	public void setTableInDB(String sTableName) throws InterruptedException {
		Thread.sleep(3000);
		txtTableName.sendKeys(sTableName);
//		UtilityCustomFunctions.sendKeys(Adriver, txtTableName, sTableName);
...
//		eleQueryTabWindow.sendKeys(sValue);
		UtilityCustomFunctions.sendKeysNoEnter(Adriver, eleQueryTabWindow, sValue);
	}
	public void setQueryInConsole(String sQuery) throws Exception {
		Thread.sleep(1000);
		System.out.println("Inside method:" + sQuery);
		if(UtilityCustomFunctions.IsElementVisible(Adriver, txtConsoleQuery)) {
			UtilityCustomFunctions.sendKeys(Adriver, txtConsoleQuery, sQuery);
			Thread.sleep(1000);
...
//		txtConsoleQuery.clear();
	...
	public void jSTypeValue(WebDriver Adriver,String sValue) throws InterruptedException {
		JavascriptExecutor js = (JavascriptExecutor) Adriver;
		js.executeScript("arguments[0].setAttribute('value','"+ sValue + "')",txtConsoleQuery);
		Thread.sleep(3000);
//		UtilityCustomFunctions.sendKeys(Adriver, txtConsoleQuery, sValue);
//		js.executeScript("document.getElementById('q')")
	...
	public void check_SN_Yearly(String sEntityId,String sModuleName,String sScheduleTypeName,String sActionType,ExtentTest node) throws InterruptedException, ParseException, IOException {
		...
		List<WebElement> eleTblRows = Adriver.findElements(By.xpath(sXpath));
		int itRowCount = eleTblRows.size();
		SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		int iDay = 0;
		int iTaskId = 0; 
		List<WebElement> tRows = Adriver.findElements(By.xpath("//table[contains(@class,'table table-striped')]/tbody/tr"));
