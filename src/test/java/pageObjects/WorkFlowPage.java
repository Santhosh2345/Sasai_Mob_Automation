...
public class WorkFlowPage extends BasePage {
	...
	public WorkFlowPage(AndroidDriver driver) {
		super(driver);
	...
	public String fWorkFlowStatus(String sModuleName,String sWorkflowName,String sExecCondition) {
		...
		for(int i=1;i<tRowCount;i++) {
			...
			WebElement eleStatus = Adriver.findElement(By.xpath(sXpath));
...
//					bWorkFlowEnabled = "true:"+j;
		...
		WebElement eleCurrentWFStatus = Adriver.findElement(By.xpath(sXpath));
		bWorkFlowEnabled = "true:"+j;
	...
	public void fClickWorkflowsList(String sModuleName) throws Exception {
		...
		boolean bFlag = UtilityCustomFunctions.IsElementVisible(Adriver, eleSelectWorkflowList);
		if(bFlag==true) {
//		UtilityCustomFunctions.doClick(Adriver, eleSelectWorflowList);
		...
		BaseClass.logger.info("Item to be selected:  " + sValue);
			UtilityCustomFunctions.selectOneItemfromListBox(Adriver, eleWorlflowUl, sValue);
	...
	public void fNavigateToTask(int iEditPos) throws Exception {
		...
		WebElement btnEditWFRow  = Adriver.findElement(By.xpath(sEditXpath));
		UtilityCustomFunctions.doClick(Adriver, btnEditWFRow);
		BaseClass.logger.info("Work flow clicked");
		System.out.println("Work flow clicked");
		Thread.sleep(1000);
		UtilityCustomFunctions.doClick(Adriver, btnstep1Next);
		...
		UtilityCustomFunctions.doClick(Adriver, btnstep2Next);
	...
	public boolean fValidateTaskStatus(String sWorkFlow,String sActionType,String sActionTitle) throws Exception {
		...
		for(int i=1;i<=tTaskRowCount;i++) {
			...
			WebElement eleTaskCheckBox = Adriver.findElement(By.xpath(sTaskXPath));
			...
			String sActActionType = UtilityCustomFunctions.getText(Adriver, tTaskColumnCount.get(1));
			String sActActionTitle = UtilityCustomFunctions.getText(Adriver, tTaskColumnCount.get(2));
